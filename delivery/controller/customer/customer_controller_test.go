...
// func (c *CustomerUseCaseMock) UpdateCustomer(customer model.Customer) error {
// 	args := c.Called(customer)
// 	if args.Get(0) != nil {
// 		return args.Get(0).(error)
// 	}
...
// 	assert.Equal(suite.T(), customers[0].Customer_name, actualCustomers[0].Customer_name)
...
// 	json.Unmarshal([]byte(response), &actualCustomers)
// 	assert.Equal(suite.T(), http.StatusOK, r.Code)
// 	assert.Equal(suite.T(), dummyCustomer.Customer_name, actualCustomers.Customer_name)
// }
